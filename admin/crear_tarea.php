...
<?php
// Este script es accesible para roles 'admin' y 'analista'.
...
unset($_SESSION['user_message'], $_SESSION['user_error'], $_SESSION['form_data']);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($nombre_tarea) || empty($fecha_vencimiento) || empty($prioridad) || empty($miembros_asignados) || empty($tipo_trabajo)) {
        $_SESSION['user_error'] = 'Todos los campos marcados con * son obligatorios.';
    ...
    if (!empty($_FILES['recursos']['name'][0])) {
        ...
        foreach ($_FILES['recursos']['name'] as $key => $name) {
            if ($_FILES['recursos']['error'][$key] == UPLOAD_ERR_OK) {
                $validacion = validar_archivo($name, $_FILES['recursos']['size'][$key]);
                if ($validacion !== true) {
                    $errores_archivos[] = "Error en el archivo '{$name}': " . $validacion;
                }
            }
        }
    ...
    try {
        ...
        $_SESSION['user_error'] = "Error al crear la tarea: " . $e->getMessage();
        header("Location: crear_tarea.php");
        exit();
    }
    ...
    if ($id_tarea) {
        ...
        if (notificar_evento_tarea($id_tarea, 'tarea_creada', $_SESSION['user_id'])) {
            $_SESSION['user_message'] = "Tarea creada y notificada exitosamente.";
        } else {
            $_SESSION['user_error'] = "La tarea fue creada, pero hubo un problema al enviar las notificaciones.";
        }
...
<div class="card form-card">
    <form action="crear_tarea.php" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <input type="text" id="nombre_tarea" name="nombre_tarea" value="<?php echo e($form_data['nombre_tarea'] ?? ''); ?>" required>
        </div>
        <div class="form-group">
            <label for="descripcion">Descripción</label>
            <textarea id="descripcion" name="descripcion" rows="4"><?php echo e($form_data['descripcion'] ?? ''); ?></textarea>
        </div>
        <div class="form-group">
            <label for="fecha_vencimiento">Fecha de Vencimiento (*)</label>
            <input type="datetime-local" id="fecha_vencimiento" name="fecha_vencimiento" value="<?php echo e($form_data['fecha_vencimiento'] ?? ''); ?>" required>
        </div>
        <div class="form-group">
            <label for="prioridad">Prioridad (*)</label>
            <select id="prioridad" name="prioridad" required>
                <option value="baja" <?php echo ($form_data['prioridad'] ?? '') == 'baja' ? 'selected' : ''; ?>>Baja</option>
                <option value="media" <?php echo ($form_data['prioridad'] ?? 'media') == 'media' ? 'selected' : ''; ?>>Media</option>
                <option value="alta" <?php echo ($form_data['prioridad'] ?? '') == 'alta' ? 'selected' : ''; ?>>Alta</option>
            </select>
        </div>
        <div class="form-group">
            <label for="numero_piezas">Número de piezas</label>
            <input type="number" id="numero_piezas" name="numero_piezas" value="<?php echo e($form_data['numero_piezas'] ?? 0); ?>" min="0">
        </div>
        <div class="form-group">
            <label for="tipo_trabajo">Tipo de Trabajo (*)</label>
            <select id="tipo_trabajo" name="tipo_trabajo" required>
                <option value="">Seleccione el tipo de trabajo</option>
                <option value="Digital" <?php echo ($form_data['tipo_trabajo'] ?? '') == 'Digital' ? 'selected' : ''; ?>>Digital</option>
                <option value="Impreso" <?php echo ($form_data['tipo_trabajo'] ?? '') == 'Impreso' ? 'selected' : ''; ?>>Impreso</option>
            </select>
        </div>
        <div class="form-group">
            ...
            <select id="negocio" name="negocio">
                ...
                <option value="Recreacion" <?php echo ($form_data['negocio'] ?? '') == 'Recreacion' ? 'selected' : ''; ?>>Recreación</option>
                <option value="Educacion" <?php echo ($form_data['negocio'] ?? '') == 'Educacion' ? 'selected' : ''; ?>>Educación</option>
                <option value="Gestion 4%" <?php echo ($form_data['negocio'] ?? '') == 'Gestion 4%' ? 'selected' : ''; ?>>Gestión 4%</option>
                <option value="MPC" <?php echo ($form_data['negocio'] ?? '') == 'MPC' ? 'selected' : ''; ?>>MPC</option>
                <option value="Credito" <?php echo ($form_data['negocio'] ?? '') == 'Credito' ? 'selected' : ''; ?>>Crédito</option>
                <option value="Interna" <?php echo ($form_data['negocio'] ?? '') == 'Interna' ? 'selected' : ''; ?>>Interna</option>
                <option value="Dirección" <?php echo ($form_data['negocio'] ?? '') == 'Dirección' ? 'selected' : ''; ?>>Dirección</option>
                <option value="Infraestructura" <?php echo ($form_data['negocio'] ?? '') == 'Infraestructura' ? 'selected' : ''; ?>>Infraestructura</option>
            </select>
        </div>
        <div class="form-group">
            ...
            <div style="max-height: 150px; overflow-y: auto; border: 1px solid #ccc; padding: 10px;">
                <?php
                $miembros_seleccionados = $form_data['miembros_asignados'] ?? [];
                foreach ($usuarios_asignables as $usuario):
                ?>
                    <div>
                        <input type="checkbox" name="miembros_asignados[]" value="<?php echo $usuario['id_usuario']; ?>" id="usuario_<?php echo $usuario['id_usuario']; ?>" <?php echo in_array($usuario['id_usuario'], $miembros_seleccionados) ? 'checked' : ''; ?>>
        ...
        <div class="form-group">
            ...
            <p class="file-upload-info">Archivos permitidos: pdf, docx, xlsx, jpg, png. Tamaño máximo por archivo: 3MB.</p>
    ...
    fileInput.addEventListener('change', function() {
        for (const file of fileInput.files) {
            ...
            if (file.size > maxSize) {
                alert(`El archivo '${file.name}' excede el tamaño máximo de 3MB.`);
                continue;
            }
