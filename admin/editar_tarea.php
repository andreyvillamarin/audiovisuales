            ...
            try {
                ...
                if (!empty($_FILES['recursos']['name'][0])) {
                    ...
                    if (!empty($errores_archivos)) {
                        throw new Exception(implode('<br>', $errores_archivos));
                    }
        ...
        if (isset($_FILES['archivos_comentario']) && !empty($_FILES['archivos_comentario']['name'][0])) {
            foreach ($_FILES['archivos_comentario']['name'] as $key => $nombre_original) {
                if ($_FILES['archivos_comentario']['error'][$key] == UPLOAD_ERR_OK) {
                    $validacion = validar_archivo($nombre_original, $_FILES['archivos_comentario']['size'][$key]);
                    if ($validacion !== true) {
                        $errores_archivos[] = "Error en el archivo '{$nombre_original}': " . $validacion;
                        continue;
                    }

                    $nombre_archivo_nuevo = time() . '_' . basename($nombre_original);
                    $ruta_destino = __DIR__ . '/../uploads/' . $nombre_archivo_nuevo;

                    if (move_uploaded_file($_FILES['archivos_comentario']['tmp_name'][$key], $ruta_destino)) {
...
<div class="grid-container">
    ...
    <div class="card">
        ...
        <form action="editar_tarea.php?id=<?php echo $id_tarea; ?>" method="POST" style="margin-top:20px;" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="archivos_comentario">Adjuntar archivos (opcional):</label>
                <p class="file-upload-info">Archivos permitidos: pdf, docx, xlsx, jpg, png. Tamaño máximo por archivo: 3MB.</p>
                <input type="file" name="archivos_comentario[]" id="archivos_comentario" multiple>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function setupFileValidation(inputId, listContainerId, selectButtonClass) {
        const fileInput = document.getElementById(inputId);
        const fileListContainer = document.getElementById(listContainerId);
        const selectFilesButton = document.querySelector(selectButtonClass);
    ...
    setupFileValidation('recursos-input', 'file-list', '.btn-select-files');
    setupFileValidation('archivos_comentario', null, null);
});
</script>
