...
function validar_archivo($archivo_nombre, $archivo_tamano) {
    $extension = strtolower(pathinfo($archivo_nombre, PATHINFO_EXTENSION));
    $extensiones_permitidas = ['pdf', 'docx', 'xlsx', 'jpg', 'png'];
    $tamano_maximo = 3 * 1024 * 1024; // 3MB

    if (!in_array($extension, $extensiones_permitidas)) {
        return "El tipo de archivo '{$extension}' no está permitido. Solo se aceptan: " . implode(', ', $extensiones_permitidas) . ".";
    }

    if ($archivo_tamano > $tamano_maximo) {
        return "El archivo excede el tamaño máximo de 3MB.";
    }
